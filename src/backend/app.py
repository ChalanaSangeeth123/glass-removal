...
def preprocess_image(image_path):
    ...
    return tf.expand_dims(img, axis=0), (original_height, original_width)
...
def postprocess_image(image, original_size):
    ...
    image = tf.image.resize(image, original_size).numpy().astype(np.uint8)
...
def upload_file():
    ...
    # Preprocess with original size tracking
    input_image, original_size = preprocess_image(filepath)
    generated_image = generator(input_image, training=False)
    result_image = postprocess_image(generated_image, original_size)
    
    result_path = os.path.join(RESULT_FOLDER, f'result_{file.filename}')
    result_image.save(result_path, quality=95)
    
    return jsonify(result_url=f"/static/results/result_{file.filename}", download_url=f"/download/result_{file.filename}")
